...
function calculateScore(seq) {
  ...
  const hydroCount = Object.entries(aaCounts).reduce((sum, [aa, count]) =>
    hydrophobic.has(aa) ? sum + count : sum, 0);
  const hydroRatio = hydroCount / length;
  const hydroScore = hydroRatio > 0.5 ? 2 : 0;

  const gpCount = Object.entries(aaCounts).reduce((sum, [aa, count]) =>
    glyPro.has(aa) ? sum + count : sum, 0);
...
function SimpleHeatmap({ sequence }) {
  ...
  const matrix = aaList.map(rowAa => positions.map(pos => sequence[pos] === rowAa ? 1 : 0));
  ...
  return (
    <div className="overflow-x-auto">
      <table className="table-auto border-collapse border border-gray-300 text-xs">
        <thead>
          <tr>
            <th className="border border-gray-300 px-1">AA</th>
            {positions.map((_, idx) => (
              <th key={idx} className="border border-gray-200 px-1">{idx + 1}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {aaList.map((aa, rowIdx) => (
            <tr key={rowIdx}>
              <td className="border border-gray-300 font-bold text-center px-1">{aa}</td>
              {matrix[rowIdx].map((val, colIdx) => (
                <td
                  key={colIdx}
                  className={`border border-gray-100 w-4 h-4 text-center ${val === 1 ? 'bg-blue-500 text-white' : ''}`}
                >
                  {val === 1 ? '■' : ''}
                </td>
              ))}
...
export default function ExpressionScorer() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">Protein Expression Feasibility Analyzer</h1>
      <Textarea
        ...
        className="mb-4"
      />
      <Button onClick={handleAnalyze}>Analyze Expression Score</Button>
        ...
        <>
          <Card className="mt-6">
            <CardContent className="p-4 space-y-2">
              ...
              <p className="text-lg"><strong>Total Expression Difficulty Score:</strong> {result.total}</p>
          ...
          <div className="mt-6">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-semibold mb-2">Amino Acid Frequencies (Bar)</h2>
              <Button onClick={() => exportToCSV(result)}>Export CSV</Button>
            </div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(result.aaCounts)
                .sort((a, b) => b[1] - a[1])
                .map(([aa, count]) => (
                  <div key={aa} className={`p-2 rounded text-center w-16 ${["S","T","Y","C","N","Q"].includes(aa) ? "bg-orange-200" : "bg-blue-100"}`}>
          ...
          {/* Amino Acid Heatmap */}
          <div className="mt-6">
            <h2 className="text-lg font-semibold mb-2">Amino Acid Distribution Heatmap</h2>
